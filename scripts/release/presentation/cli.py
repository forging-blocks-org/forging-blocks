...
def _build_parser() -> argparse.ArgumentParser:
    ...
    pr = sub.add_parser(
        "open-pr",
        help="Open the release pull request (intent to publish).",
    )
...
async def _run_open_pr(args: argparse.Namespace, container: Container) -> int:
    use_case = container.open_release_pull_request_use_case()
    output = await use_case.execute(
        OpenReleasePullRequestInput(
            base=args.base,
            head=args.head,
...
def run(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "open-pr":
            return asyncio.run(_run_open_pr(args, container))
