...
class TestGitVersionControl:
    ...
    @patch("scripts.release.infrastructure.vcs.git_version_control.run")
    def test_checkout_main_calls_git_checkout_main(self, run_mock) -> None:
        vcs = GitVersionControl()

        vcs.checkout_main()

        run_mock.assert_called_once_with(["git", "checkout", "main"])

    @patch("scripts.release.infrastructure.vcs.git_version_control.run")
    def test_delete_local_branch_calls_git_branch_d(self, run_mock) -> None:
        vcs = GitVersionControl()
        branch = ReleaseBranchName("release/v1.2.3")

        vcs.delete_local_branch(branch)

        run_mock.assert_called_once_with(["git", "branch", "-D", branch.value])

    @patch("scripts.release.infrastructure.vcs.git_version_control.run")
    def test_delete_remote_branch_calls_git_push_delete(self, run_mock) -> None:
        vcs = GitVersionControl()
        branch = ReleaseBranchName("release/v1.2.3")
        ...
        run_mock.assert_called_once_with(
            ["git", "push", "origin", "--delete", branch.value]
        )

    @patch("scripts.release.infrastructure.vcs.git_version_control.run")
    def test_delete_tag_deletes_local_and_remote_tag(self, run_mock) -> None:
        vcs = GitVersionControl()
        tag = TagName.for_version(ReleaseVersion(1, 2, 3))
        ...
        assert run_mock.call_count == 2
        run_mock.assert_any_call(["git", "tag", "-d", tag.value])
        run_mock.assert_any_call(["git", "push", "origin", "--delete", tag.value])

    @patch("scripts.release.infrastructure.vcs.git_version_control.run")
    def test_remote_branch_exists_when_exists_then_true(self, run_mock) -> None:
        vcs = GitVersionControl()
        branch = ReleaseBranchName("release/v1.2.3")

        run_mock.return_value = ""

        assert vcs.remote_branch_exists(branch) is True
    ...
    @patch("scripts.release.infrastructure.vcs.git_version_control.run")
    def test_remote_branch_exists_when_not_exists_then_false(self, run_mock) -> None:
        vcs = GitVersionControl()
        branch = ReleaseBranchName("release/v1.2.3")

        run_mock.side_effect = RuntimeError("not found")

        assert vcs.remote_branch_exists(branch) is False
