...
`auto_freeze` is an alternative to inheriting from `ValueObject`.

- If your class already has its own base class and you only need immutability — without value-based equality or hashing — prefer `@auto_freeze`.
- If you also need equality-by-value, use the [ValueObject](../domain/value-objects.md) base class instead.

Both are optional; you can use neither.
